...
contract FNFTTest is DSTest, ERC721Holder, SetupEnvironment {
    ...
    function testSwapFee() public {
        ...
        console.log("OR", originalBalance);
        console.log("OR 1", address(pair));
        console.log("OR 1", swapFee);

        fnft.transfer(address(pair), 1 ether);

        console.log("YAY");

        assertEq(fnft.balanceOf(address(pair)), 1 ether - swapFeeAmount);        
